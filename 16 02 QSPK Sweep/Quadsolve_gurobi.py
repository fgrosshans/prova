...
def QuadLyap(P, q, G, h, A, b, #Arguments for the wrapped solver
    ...
    cstate = str(h) + node # Keeping track of the whole system state. 
                           # Notice how Qt is implicit in ub: this makes the system more likely to skip optimization.
...
def UpdateConstraints(Q,node,qlabels,beta,Dt,d_arr_rates,Bt,N,
    ...
    ExpDt1 = Dt + np.array(d_arr_rates) # Initially, the constraints are the same as the no knowledge case.
